...
layui.define(["element", "jquery"], function(exports) {
    ...
    Tab.prototype.navBar = function(strData) {
            ...
            var ulHtml = '';
            for (var i = 0; i < data.length; i++) { // 第一层
                // debugger
                ...
                if (data[i].children != undefined && data[i].children.length > 0) { // 第一层 children
                    ulHtml += '<a>';
                    ...
                    for (var j = 0; j < data[i].children.length; j++) {
                        ...
                        // 如果有第三层 在 这里 拼接
                        console.info(data[i].children[j], '---')
                        if (data[i].children[j].thirdChildren && data[i].children[j].thirdChildren.length) {
                            ulHtml += "<dd class='hide' thirdChildren='true'>"
                            for (var i3 = 0; i3 < data[i].children[j].thirdChildren.length; i3++) {
                                // try {
                                //     ulHtml += '<a> ' + data[i].children[j].thirdChildren[i3].title + '</a>'
                                // } catch (err) {
